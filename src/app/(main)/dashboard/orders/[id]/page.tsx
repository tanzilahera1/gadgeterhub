...
import { 
  ...
  ShieldCheck, Info, Calendar, ShoppingBag
} from "lucide-react";
...
export default async function UserOrderDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 sm:py-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="mb-10 space-y-4">
        <Link href="/dashboard" className="inline-flex items-center gap-2 text-xs font-black text-slate-400 hover:text-primary transition-colors uppercase tracking-widest">
           <ArrowLeft className="size-3" />
           Back to Dashboard
        </Link>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
           <div className="space-y-1">
             <h1 className="text-3xl font-black tracking-tight text-slate-900">Order #{order.orderNumber}</h1>
           ...
           <div className="flex items-center gap-2 px-6 py-3 bg-primary/5 border border-primary/20 rounded-2xl">
              ...
              <span className="text-sm font-black text-primary uppercase tracking-widest">{order.orderStatus}</span>
           </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-8">
           ...
           <section className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm p-8 sm:p-12">
              ...
              <div className="relative flex items-center justify-between">
                <div className="absolute left-0 top-[1.125rem] w-full h-1 bg-slate-100 rounded-full" />
                <div 
                  className="absolute left-0 top-[1.125rem] h-1 bg-primary rounded-full transition-all duration-1000" 
                  ...
                  return (
                    <div key={step.id} className="relative z-10 flex flex-col items-center gap-4">
                       ...
                       <div className="text-center">
                         ...
                         {isCompleted && (
                           <p className="text-[8px] font-bold text-primary/60 uppercase">Completed</p>
                         )}
                       </div>
                    </div>
                  );
           ...
           <section className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden">
              ...
              <div className="divide-y divide-slate-50">
                {order.items.map((item: any, idx: number) => (
                  <div key={idx} className="p-6 sm:p-8 flex items-center gap-6 hover:bg-slate-50/30 transition-colors">
                     ...
                     <div className="flex-1 space-y-1">
                        <h4 className="text-base font-black text-slate-900 leading-tight">{item.productTitle}</h4>
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">SKU: {item.productSku}</p>
                        <div className="flex items-center gap-4 mt-2">
                           <p className="text-sm font-black text-primary">{formatPrice(item.unitPrice)}</p>
                           <span className="text-[10px] font-black bg-slate-100 px-2 py-0.5 rounded uppercase tracking-widest text-slate-500">Qty: {item.itemQuantity}</span>
                        </div>
                     </div>
              ...
              <div className="p-8 bg-slate-900 text-white flex flex-col sm:flex-row items-center justify-between gap-6">
                 <div className="flex items-center gap-6 text-center sm:text-left">
                    <div>
                      <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest mb-1">SUBTOTAL</p>
                      <p className="text-sm font-bold">{formatPrice(order.subtotal)}</p>
                    </div>
                    <div className="w-px h-8 bg-white/10 hidden sm:block" />
                    <div>
                      <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest mb-1">SHIPPING</p>
                      <p className="text-sm font-bold">{formatPrice(order.shippingCost)}</p>
                    </div>
                 </div>
                 <div className="flex items-center gap-6">
                   <div className="text-right">
                      <p className="text-[10px] font-black uppercase text-primary tracking-widest mb-1">GRAND TOTAL</p>
                      <p className="text-3xl font-black tracking-tighter">{formatPrice(order.total)}</p>
                   </div>
                 </div>
              </div>
           </section>
        ...
        <div className="space-y-8">
           ...
           <section className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm p-8 space-y-8">
              ...
              <div className="space-y-6">
                 <div className="space-y-1">
                   <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">RECEIVER NAME</p>
                   <p className="text-base font-black text-slate-900">{order.shipping.fullName}</p>
                 </div>
                 <div className="space-y-1">
                   <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">CONTACT NUMBER</p>
                   <p className="text-base font-black text-slate-900">{order.shipping.phone}</p>
                 </div>
                 <div className="space-y-1">
           ...
           <section className="bg-slate-50 rounded-[2.5rem] border border-slate-200 p-8 space-y-6 relative overflow-hidden">
              <div className="absolute top-0 right-0 size-32 bg-primary/10 rounded-full blur-[40px] -mr-16 -mt-16" />
              ...
              <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-4">
                 <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-black uppercase text-slate-900">{order.paymentMethod}</span>
                 </div>
                 <div className="flex items-center justify-between">
           ...
           <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden group">
              ...
              <p className="text-xs text-slate-400 font-medium mb-6 leading-relaxed">
                 Having trouble with your order? Our support team is available 24/7.
              </p>
