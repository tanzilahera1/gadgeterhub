      ...
      {!showAddForm && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6 bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm relative overflow-hidden group">
          ...
          <div className="flex items-center gap-4 relative z-10">
             ...
             <div>
                <h3 className="font-black text-slate-900 uppercase tracking-tight">Saved Addresses</h3>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{addresses.length} Addresses total</p>
             </div>
          </div>
          <Button 
      ...
      {showAddForm && (
        <div className="bg-white p-8 sm:p-12 rounded-[2.5rem] border border-slate-200 shadow-2xl relative animate-in zoom-in-95 duration-300">
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-xl font-black text-slate-900 tracking-tight">Add New Address</h3>
            <button onClick={() => setShowAddForm(false)} className="size-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 hover:text-slate-900 transition-colors">
              <X className="size-5" />
            </button>
          </div>
          ...
          <form onSubmit={handleAdd} className="space-y-6">
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Label (e.g. Home, Office)</label>
                  ...
                  {['Home', 'Office'].map(l => (
                    <button 
                      ...
                      className={cn(
                        ...
                        formData.label === l ? "bg-primary text-white shadow-lg" : "bg-slate-50 text-slate-400 hover:bg-slate-100"
                      )}
                    >
                      {l === 'Home' ? <Home className="size-3 inline mr-1" /> : <Briefcase className="size-3 inline mr-1" />}
                      {l}
                    </button>
                  ))}
                  <Input 
                    ...
                    value={formData.label !== 'Home' && formData.label !== 'Office' ? formData.label : ''}
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Address Details</label>
                <Input 
                  required
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">District</label>
                <Input 
                  required
                  placeholder="e.g. Dhaka"
                  className="h-14 rounded-2xl bg-slate-50 border-none font-bold text-base px-6"
            ...
            <label className="flex items-center gap-3 cursor-pointer group w-fit">
              ...
              />
              <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest group-hover:text-slate-900 transition-colors">Set as default address</span>
            </label>
            ...
            <div className="pt-6 flex gap-4">
              ...
              >
                {loading === 'adding' ? <Loader2 className="size-4 animate-spin" /> : <Plus className="size-4" />}
                Add Address
              </Button>
              <Button 
                type="button" 
      ...
      <div className="grid sm:grid-cols-2 gap-6">
        {addresses.map((addr) => (
          <div key={addr._id} className={cn(
             ...
             addr.isDefault ? "border-primary shadow-xl shadow-primary/5" : "border-slate-100 hover:border-slate-200"
          )}>
            {addr.isDefault && (
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-4">
                 <div className="size-10 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-primary/10 group-hover:text-primary transition-all">
                    {addr.label === 'Home' ? <Home className="size-5" /> : addr.label === 'Office' ? <Briefcase className="size-5" /> : <MapPin className="size-5" />}
                 </div>
                 <h4 className="text-sm font-black text-slate-900 uppercase tracking-tight">{addr.label}</h4>
              </div>

              <div className="space-y-4">
                 <div className="space-y-1">
                   <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest leading-none">Receiver</p>
                   <p className="text-sm font-black text-slate-900 truncate">{addr.fullName}</p>
                 </div>
                 <div className="space-y-1">
                   <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest leading-none">Phone</p>
                   <p className="text-sm font-bold text-slate-700">{addr.phone}</p>
                 </div>
                 <div className="space-y-1">
                   <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest leading-none">Address</p>
                   <p className="text-xs font-medium text-slate-500 leading-relaxed italic">
                     {addr.addressLine1}, {addr.district}, {addr.city} - {addr.postalCode}
                   </p>
                 </div>
              </div>
        ...
        {addresses.length === 0 && !showAddForm && (
           <div className="sm:col-span-2 text-center py-20 bg-white rounded-[2.5rem] border-2 border-dashed border-slate-100">
              ...
              <h3 className="text-lg font-black text-slate-900 mb-1">No Address Found</h3>
              <p className="text-xs font-medium text-slate-500 mb-6 uppercase tracking-widest">Add your shipping details to speed up checkout.</p>
              <Button onClick={() => setShowAddForm(true)} className="rounded-2xl h-12 px-8 font-black uppercase tracking-widest text-[10px] gap-2">
